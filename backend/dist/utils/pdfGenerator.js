...
export const generateReportPDF = (report) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.moveDown(0.5);
            doc.fontSize(10).font('Helvetica').fillColor('#666666').text(`Generated: ${new Date(report.generatedAt).toLocaleString('en-GB')}`, {
                align: 'center',
            });
            doc.moveDown(1);
            ...
            doc.moveTo(50, doc.y).lineTo(550, doc.y).stroke();
            doc.moveDown(0.5);
            const statusColor = report.verificationStatus === 'verified'
                ? '#059669'
                : report.verificationStatus === 'partial'
                    ? '#d97706'
                    : '#dc2626';
            doc.fontSize(12).font('Helvetica-Bold').fillColor(statusColor).text(report.verificationStatus.toUpperCase(), {
                align: 'center',
            });
            ...
            report.verifications.forEach((verification, index) => {
                ...
                doc.fillColor('#000000').text(new Date(verification.verifiedAt).toLocaleDateString('en-GB'), col3, currentY + 5);
            ...
            doc.moveTo(50, doc.y).lineTo(550, doc.y).stroke();
            doc.moveDown(0.5);
            const summaryText = report.verificationStatus === 'verified'
