...
const loadCandidateForReport = async (id, req) => {
    const whereClause = req.user?.role === 'admin' ? { id } : { id, createdById: req.user?.userId };
...
const buildReportData = (candidate, req) => {
...
export const downloadReportPDF = async (req, res) => {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="verification-report-${candidate.id}-${Date.now()}.pdf"`);
