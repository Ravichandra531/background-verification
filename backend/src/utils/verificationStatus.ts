export const REQUIRED_VERIFICATION_TYPES = ['aadhaar', 'pan'] as const;
...
export function getLatestLogPerType(logs: StatusLog[]): Map<string, StatusLog> {
  const latest = new Map<string, StatusLog>();
 ...
 */
export function computeCandidateStatus(logs: StatusLog[]): string {
