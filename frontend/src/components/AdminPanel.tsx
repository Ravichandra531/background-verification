...
export default function AdminPanel({ onViewCandidate }: AdminPanelProps) {
  ...
  const fetchCandidates = async () => {
    try {
      ...
      // Admin candidates search is supported via stats filtering
      ...
      console.error('Error loading global candidates:', err);
    }
  };
  ...
  const handleDeleteCandidate = async (candidateId: string) => {
    ...
    try {
      ...
      await Promise.all([fetchStatsAndUsers(), fetchCandidates()]);
    } catch (err) {
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div className="p-6 rounded-2xl border border-zinc-200/80 dark:border-zinc-800/80 bg-white/70 dark:bg-zinc-900/70 backdrop-blur-xl shadow-xl">
              ...
              <h3 className="text-2xl font-bold text-zinc-950 dark:text-zinc-50">{stats.users.total}</h3>
              <p className="text-xs text-zinc-400 font-semibold mt-1">Platform Members</p>
            </div>
            <div className="p-6 rounded-2xl border border-zinc-200/80 dark:border-zinc-800/80 bg-white/70 dark:bg-zinc-900/70 backdrop-blur-xl shadow-xl">
              <Layers className="w-5 h-5 text-indigo-500 mb-3" />
              <h3 className="text-2xl font-bold text-zinc-950 dark:text-zinc-50">{stats.candidates.total}</h3>
            ...
            <div className="p-6 rounded-2xl border border-zinc-200/80 dark:border-zinc-800/80 bg-white/70 dark:bg-zinc-900/70 backdrop-blur-xl shadow-xl">
              ...
              <h3 className="text-2xl font-bold text-zinc-950 dark:text-zinc-50">{stats.verifications.total}</h3>
          ...
          <div className="p-6 rounded-2xl border border-zinc-200/80 dark:border-zinc-800/80 bg-white/70 dark:bg-zinc-900/70 backdrop-blur-xl shadow-xl space-y-6">
            <h4 className="text-sm font-bold text-zinc-950 dark:text-zinc-50">Identity Verification Breakdown</h4>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-6 text-center">
              ...
              <div className="p-4 bg-zinc-50 dark:bg-zinc-950/30 border border-zinc-100 dark:border-zinc-800/30 rounded-xl">
                ...
                <span className="text-[10px] text-zinc-450 uppercase font-semibold mt-1 tracking-wider block">Pending</span>
              </div>
              <div className="p-4 bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/20 rounded-xl">
                ...
                <span className="text-[10px] text-zinc-450 uppercase font-semibold mt-1 tracking-wider block">Partial</span>
              </div>
              <div className="p-4 bg-rose-50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900/20 rounded-xl">
                ...
                <span className="text-[10px] text-zinc-450 uppercase font-semibold mt-1 tracking-wider block">Failed</span>
              </div>
            </div>
          </div>
        </div>
                    ...
                    <td className="py-4">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
                        u.role === 'admin' 
                          ...
                          : 'bg-zinc-150 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700/50'
                      }`}>
                        {u.role}
                      </span>
                    </td>
